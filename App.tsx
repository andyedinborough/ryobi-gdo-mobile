...
import { ryobiClient, getModuleWithVariable, getVariableFromDevice } from './RyobiClient';
...
export default class App extends React.Component<{}, AppState> {
    ...
    return (
      <View style={styles.container}>
        ...
        {devices && devices.result
          .filter(x => x.enabled && x.activated && !x.deleted && !x.metaData.description.match(/Hub/))
  ...
  private renderDevice = (device: GetDevicesResult) => {
    ...
    const state = doorState && doorState.enum && doorState.enum[doorState.value as number];
    return <View key={device._id}>
      <Text style={{ textAlign: 'center'}}>{device.metaData.name.trim()}: {state}</Text>
      <Button onPress={() => this.handleOpenPress(device.varName)} title="Open" />
      <Button onPress={() => this.handleClosePress(device.varName)} title="Close" />
      <Button onPress={() => this.handleLightOnPress(device.varName)} title="Light On" />
      <Button onPress={() => this.handleLightOffPress(device.varName)} title="Light Off" />
    </View>
  }
  ...
  private send = async (id: string, cmdName: 'light' | 'door', value: boolean) => {
    const { login, statuses } = this.state;
    const statusResult = statuses && statuses[id];
    const device = statusResult && ryobiClient.getDevice(statusResult, 'garageDoor');
    
    if(!device || !login) {
      return;
    }

    const cmd = cmdName === 'door' 
      ? ryobiClient.door(device, value)
      : ryobiClient.light(device, value);
    await ryobiClient.sendCommand(PASSWORD, login.result.auth.apiKey, cmd);
  }
  ...
  private loadDevices = async () => {
    ...
    const statusResult = await Promise.all(devices.result.map(x => ryobiClient.getStatus(x.varName)));
